    ...
    with open(CONFIG_PATH, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if line == "花朵坐标:":
                ...
                try:
                    x, y = line.split(",")
                    speed_pos = (int(x), int(y))
                except ValueError:
                    pass

    return flowers, speed_pos
...
def 加速():
    ...
    实际加速 = (speed_pos[0] + 偏移_x, speed_pos[1] + 偏移_y)
    print(f"偏移量: ({偏移_x}, {偏移_y})")
    print(f"修正后加速按钮位置: {实际加速}")
